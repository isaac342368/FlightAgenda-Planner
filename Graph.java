...
public class Graph<T> {
     ...
     */
     ...
     if (!containsVertex(start) || !containsVertex(end)) {
         throw new NoSuchElementException("start or end does not exist");
     }

     LinkedList<Vertex> visitedNodes = new LinkedList<Vertex>();
     PriorityQueue<Path> priorityQueue = new PriorityQueue<Path>();
     Vertex currVertex = vertices.get(start);
     Path shortestPath = new Path(currVertex);
     Path currPath;
     Edge currEdge = null;

     visitedNodes.add(currVertex);
     while (visitedNodes.size() < getVertexCount()) {
        for (int i = 0; i < currVertex.edgesLeaving.size(); i++) { // adds values to queue from
                                                                       // new node
           currEdge = currVertex.edgesLeaving.get(i); // Current Edge we are iterating through
           currPath = new Path(shortestPath, currEdge); // Current Path we are iterating
                                                             // through
            priorityQueue.add(currPath); // Adds current Path we are iterating through to
                                             // priority queue
         }

         if (priorityQueue.isEmpty()) { // Confirms if new values were found or link to end
                throw new NoSuchElementException("failed to find path");
         }

         while (visitedNodes.contains(priorityQueue.peek().end)) {
                priorityQueue.poll(); // While loop removes already exist end node values
         }

         if (priorityQueue.peek().end.data.equals(end)) {
            return priorityQueue.poll(); // returns if shortest path found the desired end node
         } else { // Determines next node to add values from
            visitedNodes.add(priorityQueue.peek().end); // Saves newest visited node
            shortestPath = priorityQueue.peek(); // Prepares next path to be searched
            currVertex = priorityQueue.poll().end; // Adds next node from priority queue
         }
      }
